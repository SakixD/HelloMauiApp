...
> **Abgleich mit dem aktuellen Repo-Stand (2026-07-17):** Dieser Plan wurde gegen den
> Ursprungszustand des Repos geschrieben (Branch `claude/maui-printer-library-mymu6s`, Basis =
> Initial-Commit). Der Abschnitt „Aktueller Stand" unten ist daher veraltet. Die Ziele bleiben
> gültig, die Umsetzung weicht in drei Punkten ab:
>
> 1. **Transport-Abstraktion existiert bereits**: `IPrinterConnection` +
>    `TcpPrinterConnection`/`SerialPrinterConnection`; `ZplPrinterService` arbeitet im Kern
>    schon verbindungs-agnostisch. Es wird deshalb **kein** paralleles `IPrinterTransport`
>    gebaut, sondern eine `IPrinterConnectionFactory` (Profil → `IPrinterConnection`) ergänzt.
>    `ZplPrinterService` bleibt bestehen (kein Umbenennen in `PrinterService`, kein Löschen).
> 2. **App nutzt inzwischen DI + MVVM** (CommunityToolkit.Mvvm, Registrierung in
>    `MauiProgram`): Neue Seiten entstehen als ViewModel + Fluent-Design nach dem
>    App-Schema — nicht nach dem hier beschriebenen `new`-Muster.
> 3. **Mehr Konsumenten**: 9 Stellen rufen `IPrinterService`/`IPrinterSettingsStore`
>    (ViewModels, ZplConsole, In-App-API `AppApi`, 4 alte Drill-down-Seiten). Die In-App-API
>    lernt zusätzlich `printers.list` und optionale Profil-Ids bei `print.template`/`zpl.send`;
>    das Default-Profil ist der app-weit aktive Drucker.
